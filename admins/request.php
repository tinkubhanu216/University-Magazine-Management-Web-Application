...
<body>
	...
	<div class="header">
		<center><ul class="headlink">
			...
			<li><a class="hh" href="iadmin.php">Articles</a></li>
			<li class="active1"><a class="active1" href="request.php">Requests</a></li>
		</ul></center>
	</div>
<div style="border: 0px solid grey;position: relative;width: 94%;left: 3%;right: 3%;background-color: rgb(224,224,224);margin-bottom: -20px;text-align: justify;text-justify:inter-word">
	...
	echo "	<h1>".$type."</h1>";
	echo "<form action='rresult.php' method='post'>";
	$sql = "select *from articles NATURAL JOIN ranking NATURAL JOIN astatus where contenttype='$type' and status='pending' ORDER BY title";
	...
	if (mysqli_num_rows($result) > 0) {
	    while($row = mysqli_fetch_assoc($result)){
	    	...
	    	echo "	<button class='sub' type='submit' value='".$row["artid"]."a' name='but'>accept</button>";
	    	echo "<button class='sub' type='submit' value='".$row["artid"]."d' name='but'>decline</button>";
	    	echo "</div><br><br><br>";
	    }
	}
	?>	
